...
export function buildDemoWeRunSteps(patient: Patient) {
...
function averageSteps(items: Array<{ step: number }>) {
  if (!items.length) return 0;
  return Math.round(items.reduce((sum, item) => sum + item.step, 0) / items.length);
}

export function buildMockWeRunDeviceEvent(patient: Patient): DeviceEvent {
  const steps = buildDemoWeRunSteps(patient);
  const latest = steps.at(-1) ?? { step: patient.latest.steps, timestamp: Math.round(Date.now() / 1000) };
  ...
  const riskFlags = [
    ...(readings.steps7dAvg < 3000 ? ['近7日平均步数低于3000步'] : []),
    ...(stepTrendPercent <= -35 ? ['近7日步数较前一周明显下降'] : [])
  ];
...
export const mockHealthKitPipeline: HealthKitPipeline = {
...
export function buildDemoHealthKitSamples(patient: Patient) {
  const now = Date.now();
...
export function buildMockHealthKitDeviceEvent(patient: Patient): DeviceEvent {
  const readings = {
  ...
  const riskFlags = [
    ...(readings.systolic >= 140 || readings.diastolic >= 90 ? ['Health Kit血压样本达到家庭监测高值'] : []),
    ...(readings.hrv <= 20 ? ['Health Kit HRV低于恢复基线'] : [])
  ];
  return {
    ...
    pipeline: mockHealthKitPipeline.stages
  };
}
